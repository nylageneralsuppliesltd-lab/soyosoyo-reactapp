...
export default function BalanceSheetPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const resp = await fetchWithRetry(`${API_BASE}/reports/balance-sheet?${params.toString()}`, { timeout: 15000, maxRetries: 3 });
      const json = await resp.json();
      setData(json);
    } catch (e) {
      console.debug('Balance sheet fetch failed', e);
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  const liabilities = data?.meta?.liabilities || 0;
  const equity = data?.meta?.equity || 0;

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-4">
          <ReportHeader title="Balance Sheet" subtitle={`As of: ${getPeriodLabel()}`} />
          ...
          {!loading && data && (
            <div className="statement-columns">
              ...
              <div className="statement-section">
                ...
                <div className="total-row statement-row" style={{ marginTop: '0.5rem' }}>
                  <span className="label">Liabilities + Equity</span>
                  <span className="amount">KES {(liabilities + equity).toLocaleString('en-KE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
              </div>
            </div>
          )}
